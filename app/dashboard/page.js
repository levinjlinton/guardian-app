...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (settingsRes.data) {
        ...
        setCalConnected(!!settingsRes.data.google_calendar_token);
  ...
  async function syncBlockToCalendar(block) {
    try {
                    ...
                    <>{
                      ...
                      ...dates.map((d,di)=>{
                        ...
                        return <div key={`${hour}-${di}`} className="time-cell" onClick={()=>{setBlockFormDefaults({day:di,hour});setModal('addBlock')}}>
                          {cell.map(b=><div key={b.id} className={`time-block block-${b.type}`} title={b.title} onClick={e=>{e.stopPropagation();if(calConnected)syncBlockToCalendar(b);else deleteBlock(b.id);}}>{b.title}</div>)}
                        </div>;
                      })]
                    }</>
                  ))}
          ...
          <div>
            ...
            <div style={{marginBottom:32}}>
              ...
              {calConnected
                ...
                : <div>
                    <p style={{fontSize:13,color:'var(--text-muted)',marginBottom:12}}>Connect Google Calendar to see your events in the schedule and sync blocks back.</p>
                    <a href="/api/calendar/connect" className="btn btn-primary btn-sm" style={{textDecoration:'none',display:'inline-flex',alignItems:'center',gap:6}}>
              ...
              {calEvents.length > 0 && (
                <div style={{marginTop:16}}>
                  <div style={{fontSize:11,fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:10}}>Upcoming Events (next 2 weeks)</div>
                  {calEvents.slice(0,8).map(e=>(
                    <div key={e.id} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',background:'var(--surface2)',borderRadius:8,marginBottom:6,borderLeft:'3px solid #4285F4'}}>
                      <div style={{flex:1}}>
                        <div style={{fontSize:13,fontWeight:600}}>{e.title}</div>
                        <div style={{fontSize:11,color:'var(--text-muted)'}}>{e.allDay?new Date(e.start).toLocaleDateString():new Date(e.start).toLocaleString('en',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})}</div>
                      </div>
                    </div>
                  ))}
                </div>
